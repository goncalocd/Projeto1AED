...
int ImageRegionFillingRecursive(Image img, int u, int v, uint16 label) {
  ...
  // Fill neighbors and accumulate counts
...
/// Region growing using a stack of pixel coordinates to implement the flood-filling algorithm.
int ImageRegionFillingWithSTACK(Image img, int u, int v, uint16 label) {
  ...
  // Push seed pixel onto the stack
  stack_top++;
  stack_u[stack_top] = u;
  stack_v[stack_top] = v;
  ...
  // Process pixels until the stack is empty
  while (stack_top >= 0) {
    ...
    if (ImageIsValidPixel(img, current_u, current_v) && 
      ...
      // Push all 4-connected neighbors onto stack
...
int ImageRegionFillingWithQUEUE(Image img, int u, int v, uint16 label) {
  ...
  Queue* queue = QueueCreate(img->width*2);
